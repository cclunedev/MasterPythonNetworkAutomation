...
def send_from_list(shell, commandList, timeout=1): 
  print(f'Sending command: {commandList}')
  for command in commandList:
    shell.send(command + '\n')
  time.sleep(timeout)

def send_from_file(shell, commandFile, timeout=1):
    with open(commandFile, 'r') as commandsFile:
      commandList = commandsFile.readlines()
    for command in commandList:
      shell.send(command)
    time.sleep(timeout)
